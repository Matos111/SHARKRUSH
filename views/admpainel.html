<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SharkRush - Painel Administrativo</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
    <style>
        * {
            margin: 0;
            padding: 0;
            box-sizing: border-box;
        }

        body {
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            background: radial-gradient(ellipse at center, #1a0000 0%, #000000 100%);
            min-height: 100vh;
            overflow-x: hidden;
            position: relative;
        }

        /* Fundo animado */
        .animated-bg {
            position: fixed;
            top: 0;
            left: 0;
            width: 100vw;
            height: 100vh;
            z-index: 0;
            pointer-events: none;
            overflow: hidden;
        }

        .particle {
            position: absolute;
            background: #FF1F1F;
            border-radius: 50%;
            opacity: 0.4;
            animation: float 8s ease-in-out infinite;
        }

        @keyframes float {
            0%, 100% { transform: translateY(0px) rotate(0deg); }
            50% { transform: translateY(-20px) rotate(180deg); }
        }

        .wave {
            position: absolute;
            left: 0;
            width: 100vw;
            height: 120px;
            background: linear-gradient(90deg, transparent, rgba(255, 31, 31, 0.07), transparent);
            opacity: 0.7;
            animation: wave 12s linear infinite;
        }

        .wave:nth-child(1) { top: 20%; animation-delay: 0s; }
        .wave:nth-child(2) { top: 40%; animation-delay: -4s; }
        .wave:nth-child(3) { top: 60%; animation-delay: -8s; }

        @keyframes wave {
            0% { transform: translateX(-100%); }
            100% { transform: translateX(100%); }
        }

        /* Modal de autenticação */
        .auth-overlay {
            position: fixed;
            top: 0;
            left: 0;
            width: 100vw;
            height: 100vh;
            background: rgba(0, 0, 0, 0.95);
            display: flex;
            align-items: center;
            justify-content: center;
            z-index: 2000;
            backdrop-filter: blur(10px);
        }

        .auth-overlay.hidden {
            display: none;
        }

        .auth-container {
            background: rgba(30, 30, 30, 0.25);
            backdrop-filter: blur(24px) saturate(180%) brightness(1.1);
            border: 1.5px solid rgba(255, 255, 255, 0.18);
            box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37), 0 0 0 2px rgba(255,31,31,0.12);
            border-radius: 24px;
            padding: 3rem;
            width: 100%;
            max-width: 450px;
            margin: 1rem;
            animation: slideIn 0.8s ease-out;
            position: relative;
            overflow: hidden;
        }

        .auth-container::before {
            content: '';
            position: absolute;
            top: -50%;
            left: -50%;
            width: 200%;
            height: 200%;
            background: conic-gradient(from 0deg, transparent, rgba(255, 31, 31, 0.1), transparent);
            animation: rotate 10s linear infinite;
            z-index: -1;
        }

        @keyframes slideIn {
            from {
                opacity: 0;
                transform: translateY(50px);
            }
            to {
                opacity: 1;
                transform: translateY(0);
            }
        }

        @keyframes rotate {
            from { transform: rotate(0deg); }
            to { transform: rotate(360deg); }
        }

        .auth-header {
            text-align: center;
            margin-bottom: 2rem;
        }

        .lock-icon {
            width: 80px;
            height: 80px;
            margin: 0 auto 1.5rem;
            background: linear-gradient(135deg, rgba(255, 31, 31, 0.2), rgba(255, 31, 31, 0.05));
            border-radius: 50%;
            display: flex;
            align-items: center;
            justify-content: center;
            border: 2px solid rgba(255, 31, 31, 0.3);
            animation: pulse 2s ease-in-out infinite;
        }

        .lock-icon i {
            font-size: 2.5rem;
            color: #FF1F1F;
            text-shadow: 0 0 20px rgba(255, 31, 31, 0.5);
        }

        @keyframes pulse {
            0%, 100% { transform: scale(1); box-shadow: 0 0 20px rgba(255, 31, 31, 0.3); }
            50% { transform: scale(1.05); box-shadow: 0 0 30px rgba(255, 31, 31, 0.5); }
        }

        .auth-title {
            font-size: 1.8rem;
            font-weight: bold;
            color: #fff;
            margin-bottom: 0.5rem;
        }

        .auth-subtitle {
            color: #ccc;
            font-size: 0.95rem;
        }

        .auth-form {
            display: flex;
            flex-direction: column;
            gap: 1.5rem;
        }

        .form-group {
            position: relative;
        }

        .form-label {
            color: #ccc;
            font-size: 0.9rem;
            margin-bottom: 0.5rem;
            display: block;
        }

        .form-input {
            width: 100%;
            padding: 1rem 1rem 1rem 3rem;
            background: rgba(255, 255, 255, 0.05);
            border: 2px solid rgba(255, 31, 31, 0.3);
            border-radius: 12px;
            color: white;
            font-size: 1rem;
            transition: all 0.3s ease;
            outline: none;
        }

        .form-input:focus {
            border-color: #FF1F1F;
            box-shadow: 0 0 20px rgba(255, 31, 31, 0.3);
            background: rgba(255, 255, 255, 0.1);
        }

        .input-icon {
            position: absolute;
            left: 1rem;
            bottom: 1rem;
            color: #FF1F1F;
            font-size: 1.2rem;
        }

        .btn {
            width: 100%;
            padding: 1rem;
            border: none;
            border-radius: 12px;
            font-size: 1.1rem;
            font-weight: bold;
            cursor: pointer;
            transition: all 0.3s ease;
            position: relative;
            overflow: hidden;
            background: linear-gradient(135deg, #FF1F1F, #cc0000);
            color: white;
        }

        .btn::before {
            content: '';
            position: absolute;
            top: 0;
            left: -100%;
            width: 100%;
            height: 100%;
            background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.2), transparent);
            transition: left 0.5s ease;
        }

        .btn:hover::before {
            left: 100%;
        }

        .btn:hover {
            transform: translateY(-2px);
            box-shadow: 0 10px 25px rgba(255, 31, 31, 0.4);
        }

        .error-message {
            color: #ff4444;
            font-size: 0.85rem;
            margin-top: 0.5rem;
            text-align: center;
            opacity: 0;
            transform: translateY(-10px);
            transition: all 0.3s ease;
        }

        .error-message.show {
            opacity: 1;
            transform: translateY(0);
        }

        /* NAVBAR */
        .main-menu {
            background: linear-gradient(180deg, #232323 0%, #1a1a1a 100%);
            position: fixed;
            top: 0;
            bottom: 0;
            height: 100%;
            left: 0;
            width: 70px;
            overflow: hidden;
            transition: width 0.3s cubic-bezier(0.4, 0, 0.2, 1);
            box-shadow: 2px 0 20px rgba(0, 0, 0, 0.15);
            z-index: 1002;
        }

        /* make sidebar a column so we can pin logout to bottom */
        .main-menu {
            display: flex;
            flex-direction: column;
        }

        .main-menu:hover {
            width: 280px;
            box-shadow: 2px 0 25px rgba(255, 0, 0, 0.15);
        }

        .main-menu ul {
            margin: 7px 0;
            padding: 0;
            list-style: none;
        }

        /* make nav list grow so items can be spaced and a bottom item can stick */
        .main-menu ul {
            display: flex;
            flex-direction: column;
            flex: 1 1 auto;
        }

        .main-menu li {
            position: relative;
            display: block;
            width: 250px;
        }

        .main-menu li a {
            position: relative;
            width: 100%;
            display: table;
            color: #c5c5c5;
            font-size: 16px;
            text-decoration: none;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
            border-top: 1px solid rgba(78, 78, 78, 0.2);
            padding: 10px 0;
            height: 55px;
            overflow: hidden;
        }

        .main-menu .nav-icon {
            position: relative;
            display: table-cell;
            width: 70px;
            height: 55px;
            text-align: center;
            vertical-align: middle;
            font-size: 26px;
            padding: 12px 0;
            transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .main-menu .nav-text {
            position: relative;
            display: table-cell;
            vertical-align: middle;
            width: 190px;
            font-size: 16px;
            padding-left: 15px;
            opacity: 0;
            transform: translateX(-10px);
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .main-menu:hover .nav-text {
            opacity: 1;
            transform: translateX(0);
        }

        .main-menu li:hover > a {
            color: #ffffff;
            background: linear-gradient(45deg, #323232 0%, #2b2b2b 100%);
            transform: translateX(8px);
            box-shadow: -5px 0 15px rgba(0, 0, 0, 0.2);
        }

        .main-menu li:hover .nav-icon {
            transform: scale(1.15);
            text-shadow: 0 0 10px rgba(175, 175, 175, 0.5);
        }

        .main-menu li a::before {
            content: '';
            position: absolute;
            left: 0;
            top: 0;
            width: 3px;
            height: 100%;
            background: #ff0000;
            transform: scaleY(0);
            transition: transform 0.3s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .main-menu li:hover a::before {
            transform: scaleY(1);
        }

        .main-menu li a.active {
            background: linear-gradient(45deg, #373737 0%, #292929 100%);
            color: #ffffff;
            box-shadow: inset 0 0 15px rgba(0, 0, 0, 0.2);
        }

        .main-menu li a.active::before {
            transform: scaleY(1);
        }

        .main-menu li a.active .nav-icon {
            transform: scale(1.15);
            text-shadow: 0 0 10px rgba(255, 255, 255, 0.5);
        }

        /* style for login/logout special link (dark) */
        .main-menu li a.nav-login {
            background: #2f2f2f;
            color: #d6d6d6;
        }
        .main-menu li a.nav-login .nav-icon {
            color: #d6d6d6;
        }
        .main-menu li a.nav-login:hover {
            background: #3b3b3b;
            color: #ffffff;
        }

        /* pin nav-bottom items to footer area */
        .main-menu li.nav-bottom {
            margin-top: auto;
            padding-bottom: 12px;
        }

        .main-menu li.nav-bottom a {
            border-top: 1px solid rgba(255,255,255,0.04);
            background: linear-gradient(180deg, rgba(47,47,47,0.95), rgba(39,39,39,0.95));
            padding-top: 12px;
            padding-bottom: 12px;
        }

        .logo-container {
            height: 100px;
            display: flex;
            align-items: center;
            justify-content: center;
            padding: 2px;
        }

        .logo-container img {
            max-width: 100%;
            max-height: 120px;
        }

        /* Conteúdo principal */
        .main-content {
            margin-left: 70px;
            padding: 2rem;
            position: relative;
            z-index: 1;
        }

        .admin-header {
            background: rgba(30, 30, 30, 0.25);
            backdrop-filter: blur(24px);
            border: 1.5px solid rgba(255, 255, 255, 0.18);
            border-radius: 24px;
            padding: 2rem;
            margin-bottom: 2rem;
            box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37);
        }

        .admin-title {
            font-size: 2rem;
            color: #fff;
            margin-bottom: 0.5rem;
            display: flex;
            align-items: center;
            gap: 1rem;
        }

        .admin-title i {
            color: #FF1F1F;
            text-shadow: 0 0 20px rgba(255, 31, 31, 0.5);
        }

        .admin-subtitle {
            color: #ccc;
            font-size: 1rem;
        }

        .stats-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(250px, 1fr));
            gap: 1.5rem;
            margin-bottom: 2rem;
        }

        .stat-card {
            background: rgba(30, 30, 30, 0.25);
            backdrop-filter: blur(24px);
            border: 1.5px solid rgba(255, 255, 255, 0.18);
            border-radius: 16px;
            padding: 1.5rem;
            transition: all 0.3s ease;
            position: relative;
            overflow: hidden;
        }

        .stat-card::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            width: 100%;
            height: 4px;
            background: linear-gradient(90deg, #FF1F1F, #cc0000);
        }

        .stat-card:hover {
            transform: translateY(-5px);
            box-shadow: 0 10px 30px rgba(255, 31, 31, 0.3);
        }

        .stat-icon {
            width: 50px;
            height: 50px;
            background: linear-gradient(135deg, rgba(255, 31, 31, 0.2), rgba(255, 31, 31, 0.05));
            border-radius: 12px;
            display: flex;
            align-items: center;
            justify-content: center;
            margin-bottom: 1rem;
        }

        .stat-icon i {
            font-size: 1.5rem;
            color: #FF1F1F;
        }

        .stat-value {
            font-size: 2rem;
            font-weight: bold;
            color: #fff;
            margin-bottom: 0.5rem;
        }

        .stat-label {
            color: #ccc;
            font-size: 0.9rem;
        }

        .clients-container {
            background: rgba(30, 30, 30, 0.25);
            backdrop-filter: blur(24px);
            border: 1.5px solid rgba(255, 255, 255, 0.18);
            border-radius: 24px;
            padding: 2rem;
            box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37);
        }

        .search-bar {
            display: flex;
            gap: 1rem;
            margin-bottom: 2rem;
        }

        .search-input {
            flex: 1;
            padding: 1rem 1rem 1rem 3rem;
            background: rgba(255, 255, 255, 0.05);
            border: 2px solid rgba(255, 31, 31, 0.3);
            border-radius: 12px;
            color: white;
            font-size: 1rem;
            outline: none;
            transition: all 0.3s ease;
            position: relative;
        }

        .search-input:focus {
            border-color: #FF1F1F;
            box-shadow: 0 0 20px rgba(255, 31, 31, 0.3);
        }

        .search-wrapper {
            position: relative;
            flex: 1;
        }

        .search-wrapper i {
            position: absolute;
            left: 1rem;
            top: 50%;
            transform: translateY(-50%);
            color: #FF1F1F;
            font-size: 1.2rem;
        }

        .filter-btn {
            padding: 1rem 2rem;
            background: rgba(255, 255, 255, 0.05);
            border: 2px solid rgba(255, 31, 31, 0.3);
            border-radius: 12px;
            color: #ccc;
            cursor: pointer;
            transition: all 0.3s ease;
        }

        .filter-btn:hover {
            background: rgba(255, 31, 31, 0.1);
            color: #fff;
            border-color: #FF1F1F;
        }

        .clients-table {
            width: 100%;
            border-collapse: collapse;
            margin-top: 1rem;
        }

        .clients-table thead {
            background: rgba(255, 31, 31, 0.1);
        }

        .clients-table th {
            padding: 1rem;
            text-align: left;
            color: #FF1F1F;
            font-weight: 600;
            border-bottom: 2px solid rgba(255, 31, 31, 0.3);
        }

        .clients-table td {
            padding: 1rem;
            color: #ccc;
            border-bottom: 1px solid rgba(255, 255, 255, 0.1);
        }

        .clients-table tbody tr {
            transition: all 0.3s ease;
        }

        .clients-table tbody tr:hover {
            background: rgba(255, 31, 31, 0.05);
        }

        .status-badge {
            padding: 0.4rem 0.8rem;
            border-radius: 20px;
            font-size: 0.85rem;
            font-weight: 600;
            display: inline-block;
        }

        .status-active {
            background: rgba(68, 255, 68, 0.2);
            color: #44ff44;
            border: 1px solid rgba(68, 255, 68, 0.3);
        }

        .status-inactive {
            background: rgba(255, 68, 68, 0.2);
            color: #ff4444;
            border: 1px solid rgba(255, 68, 68, 0.3);
        }

        .action-btn {
            padding: 0.5rem 1rem;
            background: rgba(255, 255, 255, 0.05);
            border: 1px solid rgba(255, 31, 31, 0.3);
            border-radius: 8px;
            color: #ccc;
            cursor: pointer;
            transition: all 0.3s ease;
            margin: 0 0.2rem;
        }

        .action-btn:hover {
            background: rgba(255, 31, 31, 0.1);
            color: #fff;
            border-color: #FF1F1F;
        }

        @media (max-width: 768px) {
            .main-content {
                margin-left: 0;
                padding: 1rem;
            }

            .stats-grid {
                grid-template-columns: 1fr;
            }

            .search-bar {
                flex-direction: column;
            }

            .clients-table {
                font-size: 0.85rem;
            }

            .clients-table th,
            .clients-table td {
                padding: 0.75rem 0.5rem;
            }
        }
    </style>
</head>
<body>
    <!-- Background animado -->
    <div class="animated-bg">
        <div class="wave"></div>
        <div class="wave"></div>
        <div class="wave"></div>
    </div>

    <!-- Modal de autenticação -->
    <div class="auth-overlay" id="authOverlay">
        <div class="auth-container">
            <div class="auth-header">
                <div class="lock-icon">
                    <i class="fas fa-shield-halved"></i>
                </div>
                <h1 class="auth-title">Acesso Administrativo</h1>
                <p class="auth-subtitle">Digite a senha de administrador para continuar</p>
            </div>

            <form class="auth-form" id="authForm">
                <div class="form-group">
                    <label class="form-label">Senha de Administrador</label>
                    <input type="password" class="form-input" id="adminPassword" placeholder="Digite a senha" autocomplete="off">
                    <i class="fas fa-lock input-icon"></i>
                </div>

                <button type="submit" class="btn">
                    <i class="fas fa-unlock-alt"></i> Acessar Painel
                </button>

                <div class="error-message" id="authError"></div>
            </form>
        </div>
    </div>

    <!-- NAVBAR -->
    <nav class="main-menu">
        <div class="logo-container">
            <img src="../views/midia/Logos/logoshark.png" alt="Logo" />
        </div>
        <ul>
            <li>
                <a href="../views/adminhome.html">
                    <i class="fa fa-home nav-icon"></i>
                    <span class="nav-text">Home</span>
                </a>
            </li>
            

            
            <li>
                <a href="../views/admpainel.html" class="active">
                    <i class="fa fa-users-cog nav-icon"></i>
                    <span class="nav-text">Painel Admin</span>
                </a>
            </li>
            <li>
                <a href="../views/admbiblioteca.html">
                    <i class="fa fa-book nav-icon"></i>
                    <span class="nav-text">Biblioteca</span>
                </a>
            </li>
            <li class="nav-bottom">
                <a href="../views/login.html" class="nav-login" id="logoutBtn" onclick="logout(event)">
                    <i class="fa fa-sign-out-alt nav-icon"></i>
                    <span class="nav-text">Sair</span>
                </a>
            </li>
        </ul>
    </nav>

    <!-- Conteúdo principal -->
    <div class="main-content" id="mainContent" style="display:none;">
        <div class="admin-header">
            <h1 class="admin-title">
                <i class="fas fa-users-cog"></i>
                Painel Administrativo
            </h1>
            <p class="admin-subtitle">Gerencie todos os clientes cadastrados no sistema</p>
        </div>

        <!-- Estatísticas -->
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-icon">
                    <i class="fas fa-users"></i>
                </div>
                <div class="stat-value" id="totalClients">0</div>
                <div class="stat-label">Total de Clientes</div>
            </div>

            <div class="stat-card">
                <div class="stat-icon">
                    <i class="fas fa-user-check"></i>
                </div>
                <div class="stat-value" id="activeClients">0</div>
                <div class="stat-label">Clientes Ativos</div>
            </div>

            <div class="stat-card">
                <div class="stat-icon">
                    <i class="fas fa-user-clock"></i>
                </div>
                <div class="stat-value" id="inactiveClients">0</div>
                <div class="stat-label">Clientes Inativos</div>
            </div>

            <div class="stat-card">
                <div class="stat-icon">
                    <i class="fas fa-calendar-plus"></i>
                </div>
                <div class="stat-value" id="newClients">0</div>
                <div class="stat-label">Novos (30 dias)</div>
            </div>
        </div>

        <!-- Container de clientes -->
        <div class="clients-container">
            <div class="search-bar">
                <div class="search-wrapper">
                    <i class="fas fa-search"></i>
                    <input type="text" class="search-input" id="searchInput" placeholder="Buscar por nome, email ou telefone...">
                </div>
                <button class="filter-btn" id="filterBtn">
                    <i class="fas fa-filter"></i> Filtros
                </button>
            </div>

            <table class="clients-table">
                <thead>
                    <tr>
                        <th>ID</th>
                        <th>Nome</th>
                        <th>Email</th>
                        <th>Telefone</th>
                        <th>Data Cadastro</th>
                        <th>Status</th>
                        <th>Ações</th>
                    </tr>
                </thead>
                <tbody id="clientsTableBody">
                    <!-- Dados serão inseridos via JavaScript -->
                </tbody>
            </table>
        </div>
    </div>

    <script>
        // Senha de administrador (em produção, isso deveria ser verificado no backend)
        const ADMIN_PASSWORD = 'admin123';

        // Dados de exemplo dos clientes
        const clientsData = [
            { id: 1, name: 'João Silva', email: 'joao.silva@email.com', phone: '(11) 98765-4321', date: '2024-10-15', status: 'active' },
            { id: 2, name: 'Maria Santos', email: 'maria.santos@email.com', phone: '(11) 97654-3210', date: '2024-10-20', status: 'active' },
            { id: 3, name: 'Pedro Oliveira', email: 'pedro.oliveira@email.com', phone: '(11) 96543-2109', date: '2024-09-10', status: 'inactive' },
            { id: 4, name: 'Ana Costa', email: 'ana.costa@email.com', phone: '(11) 95432-1098', date: '2024-10-25', status: 'active' },
            { id: 5, name: 'Carlos Ferreira', email: 'carlos.ferreira@email.com', phone: '(11) 94321-0987', date: '2024-08-05', status: 'inactive' },
            { id: 6, name: 'Juliana Lima', email: 'juliana.lima@email.com', phone: '(11) 93210-9876', date: '2024-10-28', status: 'active' },
            { id: 7, name: 'Ricardo Souza', email: 'ricardo.souza@email.com', phone: '(11) 92109-8765', date: '2024-10-01', status: 'active' },
            { id: 8, name: 'Fernanda Alves', email: 'fernanda.alves@email.com', phone: '(11) 91098-7654', date: '2024-07-15', status: 'inactive' },
        ];

        let isAuthenticated = false;

        // Criar partículas
        function createParticles() {
            const animatedBg = document.querySelector('.animated-bg');
            const particleCount = 40;
            for (let i = 0; i < particleCount; i++) {
                const particle = document.createElement('div');
                particle.className = 'particle';
                const size = Math.random() * 3 + 2;
                particle.style.width = `${size}px`;
                particle.style.height = `${size}px`;
                particle.style.left = `${Math.random() * 100}%`;
                particle.style.top = `${Math.random() * 100}%`;
                particle.style.animationDelay = `${Math.random() * 8}s`;
                particle.style.animationDuration = `${8 + Math.random() * 4}s`;
                animatedBg.appendChild(particle);
            }
        }

        // Autenticação
        document.getElementById('authForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const password = document.getElementById('adminPassword').value;
            const errorElement = document.getElementById('authError');

            if (password === ADMIN_PASSWORD) {
                isAuthenticated = true;
                document.getElementById('authOverlay').classList.add('hidden');
                document.getElementById('mainContent').style.display = 'block';
                loadClientsData();
            } else {
                errorElement.textContent = 'Senha incorreta. Tente novamente.';
                errorElement.classList.add('show');
                setTimeout(() => errorElement.classList.remove('show'), 2000);
            }
        });

        // Carregar dados dos clientes na tabela
        function loadClientsData(filteredClients = null) {
            const tbody = document.getElementById('clientsTableBody');
            const data = filteredClients || clientsData;
            tbody.innerHTML = '';
            data.forEach(client => {
                tbody.innerHTML += `
                    <tr>
                        <td>${client.id}</td>
                        <td>${client.name}</td>
                        <td>${client.email}</td>
                        <td>${client.phone}</td>
                        <td>${client.date}</td>
                        <td><span class="status-badge status-${client.status}">${client.status === 'active' ? 'Ativo' : 'Inativo'}</span></td>
                        <td>
                            <button class="action-btn" onclick="editClient(${client.id})"><i class="fas fa-edit"></i> Editar</button>
                            <button class="action-btn" onclick="deleteClient(${client.id})"><i class="fas fa-trash"></i> Excluir</button>
                        </td>
                    </tr>
                `;
            });
            updateStats(data);
        }

        // Atualizar estatísticas
        function updateStats(data) {
            document.getElementById('totalClients').textContent = data.length;
            document.getElementById('activeClients').textContent = data.filter(c => c.status === 'active').length;
            document.getElementById('inactiveClients').textContent = data.filter(c => c.status === 'inactive').length;
            // Novos clientes nos últimos 30 dias
            const now = new Date();
            const newClients = data.filter(c => {
                const date = new Date(c.date);
                return (now - date) / (1000 * 60 * 60 * 24) <= 30;
            });
            document.getElementById('newClients').textContent = newClients.length;
        }

        // Buscar clientes
        document.getElementById('searchInput').addEventListener('input', function() {
            const query = this.value.toLowerCase();
            const filtered = clientsData.filter(c =>
                c.name.toLowerCase().includes(query) ||
                c.email.toLowerCase().includes(query) ||
                c.phone.toLowerCase().includes(query)
            );
            loadClientsData(filtered);
        });

        // Filtro (exemplo: alternar ativos/inativos)
        document.getElementById('filterBtn').addEventListener('click', function() {
            const current = this.dataset.filter || 'all';
            let filtered;
            if (current === 'all' || !current) {
                filtered = clientsData.filter(c => c.status === 'active');
                this.textContent = 'Mostrar Todos';
                this.dataset.filter = 'active';
            } else if (current === 'active') {
                filtered = clientsData.filter(c => c.status === 'inactive');
                this.textContent = 'Mostrar Todos';
                this.dataset.filter = 'inactive';
            } else {
                filtered = clientsData;
                this.textContent = 'Filtros';
                this.dataset.filter = 'all';
            }
            loadClientsData(filtered);
        });

        // Editar cliente (apenas exibe alerta)
        window.editClient = function(id) {
            const client = clientsData.find(c => c.id === id);
            if (client) {
                alert(`Editar cliente: ${client.name}`);
            }
        };

        // Excluir cliente
        window.deleteClient = function(id) {
            if (confirm('Tem certeza que deseja excluir este cliente?')) {
                const idx = clientsData.findIndex(c => c.id === id);
                if (idx !== -1) {
                    clientsData.splice(idx, 1);
                    loadClientsData();
                }
            }
        };

        // Inicialização
        window.onload = function() {
            createParticles();
            // Garante que overlay de autenticação está visível ao carregar
            document.getElementById('authOverlay').classList.remove('hidden');
            // NÃO oculta mainContent aqui, pois isso sobrescreve display após login
        };
        
        // Logout helper: clears storage and redirects to login
        function logout(e) {
            if (e && typeof e.preventDefault === 'function') e.preventDefault();
            try {
                localStorage.clear();
                sessionStorage.clear();
            } catch (err) {
                console.warn('Logout: could not clear storage', err);
            }
            window.location.href = '../senaviews/login.html';
        }
        </script>
</body>